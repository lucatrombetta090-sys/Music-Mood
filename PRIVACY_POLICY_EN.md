...
- year, if available
- file path or folder
- local artwork, if available
- local song identifier

This data is used to display the library, organize songs, play music, analyze audio content, generate statistics and create music reports.
...
- estimated BPM
- musical key
- major or minor mode, if available
- optional mood manually changed by the user

This data is used only for internal app features, such as library, player, BubbleMap, statistics, music profile, automatic playlists and Weekly Mood Report.
...
- analyzed song count
- daily music suggestions

This data is generated locally and used only for app features.
...
- aggregated Last.fm statistics shown in the Weekly Report
...
- artist
- album
- listening time, if available
- Last.fm track URL, if available
- image or artwork URL, if available
...
- allow manual mood corrections
- calculate valence, arousal, BPM and key
- generate the BubbleMap
- calibrate the classifier
- generate music statistics
- generate the personal music profile
...
Data remains on the device until the user deletes app data, uninstalls the app or uses available deletion features.
...
The user can revoke permissions from Android device settings.
...
Music-Mood is not specifically designed for children and does not knowingly intend to collect personal data from children.
...
- uninstall the app
- avoid configuring Last.fm
- remove or modify Last.fm credentials
- remove Last.fm cache, if available
