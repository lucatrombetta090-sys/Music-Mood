...
object SongCache {
    ...
    private const val CACHE_VER   = 5   // incrementato: aggiunto coverPath
        ...
        try {
            ...
            songs.forEach { s ->
                arr.put(JSONObject().apply {
                    ...
                    put("moodOverride",  s.moodOverride)
                    put("coverPath",     s.coverPath)
                })
            }
            val root = JSONObject()
        ...
        return try {
            ...
            (0 until arr.length()).map { i ->
                ...
                Song(
                    ...
                    moodOverride  = o.optString("moodOverride", ""),
    ...
    fun saveMoodOverride(context: Context, path: String, mood: String) {
        val key = "mood_override_" + path.hashCode()
        prefs(context).edit().putString(key, mood).apply()
    }

    fun loadMoodOverrides(context: Context): Map<String, String> {
        val p = prefs(context)
        return p.all.entries
            .filter { it.key.startsWith("mood_override_") }
            .associate { it.key to (it.value as? String ?: "") }
            .filterValues { it.isNotBlank() }
    }

    fun getMoodOverride(context: Context, path: String): String? {
        val key = "mood_override_" + path.hashCode()
        return prefs(context).getString(key, null)
    }

    fun clearMoodOverride(context: Context, path: String) {
        val key = "mood_override_" + path.hashCode()
